...
public class BrunomateusRequestHandler extends SimpleRequestHandler{
	...
	private final static int[]brunomateusOidRep = {1,3,6,1,4,1,1};

	/**
	 * To get OID array of this Scalar Group
	* @return OID array of this Scalar Group
	*/
	public static int[] getBrunomateusOidRep(){
		return brunomateusOidRep;
	}

	// This is generated to preserve the old API
	public int[] getOidRep(){
		return getBrunomateusOidRep();
	}
	public SnmpAgent_brunomateus agentName;
	...
	*/
	...
	BrunomateusInstrument instrument = null;

	public void setBrunomateusInstrument(BrunomateusInstrument instrument){
		this.instrument = instrument;
		this.addInstrumentHandler(instrument);
	}

	public BrunomateusRequestHandler(SnmpAgent_brunomateus agentRef){
		agentName = agentRef;
	}
	...
	*/
	public Hashtable getObjectHash(){
		if(objectHash == null){
			...
			objectHash.put("QUERYCOUNTER",new Integer(1));
			objectHash.put("SCREENRESOLUTIONWIDTH",new Integer(2));
			objectHash.put("SCREENRESOLUTIONHEIGHT",new Integer(3));
			objectHash.put("FOLDERTOCOUNT",new Integer(4));
			objectHash.put("FILESINFOLDER",new Integer(5));
			objectHash.put("FILESYSTEMCOUNT",new Integer(6));
		}
		return objectHash;
	}
	...
	*/
	public Hashtable getObjectTypeHash(){
		if(objectTypeHash == null){
			...
			objectTypeHash.put("QUERYCOUNTER",new Integer(66));
			objectTypeHash.put("SCREENRESOLUTIONWIDTH",new Integer(2));
			objectTypeHash.put("SCREENRESOLUTIONHEIGHT",new Integer(2));
			objectTypeHash.put("FOLDERTOCOUNT",new Integer(4));
			objectTypeHash.put("FILESINFOLDER",new Integer(2));
			objectTypeHash.put("FILESYSTEMCOUNT",new Integer(66));
		}
	...
	protected void processGetRequest(Vector listOfVarbinds,
		...
		for(int i=0;i<nodeSize;i++){
			try{
				...
				if(oid.length != brunomateusOidRep.length + 2){
					AgentUtil.throwNoSuchInstance(pe.getVersion());
				}

				int[] inst = AgentUtil.getInstance(oid,brunomateusOidRep.length);
				if(inst[1] != 0){
					AgentUtil.throwNoSuchInstance(pe.getVersion());
				}
			...
			try{
				...
				switch(node.getSubId()){
					...
					case FOLDERTOCOUNT:	  
						...
						String folderToCountStr = (String )var.getVarObject();
						toSendHash.put(new Integer(FOLDERTOCOUNT),folderToCountStr);
						indexList.removeElement(new Integer(FOLDERTOCOUNT));
						indexList.addElement(new Integer(FOLDERTOCOUNT));
						break;
	...
	public void getValuesFromInstrument(VarBindRequestEvent pe, Vector varbVector,Vector columns,Vector failedList) {
		...
		for(int j=0; j< size; j++){
			try{
				...
				switch(columnID){
					case QUERYCOUNTER:	  
						toReturn = valuesHash.get(new Integer(QUERYCOUNTER));
						if(toReturn == null){
							AgentUtil.throwNoSuchInstance(pe.getVersion());
						}
						SnmpGauge var0 = new SnmpGauge(((Long )toReturn).longValue());
						varb.setVariable(var0);
						break;

					case SCREENRESOLUTIONWIDTH:	  
						toReturn = valuesHash.get(new Integer(SCREENRESOLUTIONWIDTH));
						if(toReturn == null){
							AgentUtil.throwNoSuchInstance(pe.getVersion());
						}
					...
					case SCREENRESOLUTIONHEIGHT:	  
						toReturn = valuesHash.get(new Integer(SCREENRESOLUTIONHEIGHT));
						if(toReturn == null){
							AgentUtil.throwNoSuchInstance(pe.getVersion());
						}
					...
					case FOLDERTOCOUNT:	  
						toReturn = valuesHash.get(new Integer(FOLDERTOCOUNT));
						if(toReturn == null){
							AgentUtil.throwNoSuchInstance(pe.getVersion());
						}
					...
					case FILESINFOLDER:	  
						toReturn = valuesHash.get(new Integer(FILESINFOLDER));
						if(toReturn == null){
							AgentUtil.throwNoSuchInstance(pe.getVersion());
						}
					...
					case FILESYSTEMCOUNT:	  
						toReturn = valuesHash.get(new Integer(FILESYSTEMCOUNT));
						if(toReturn == null){
							AgentUtil.throwNoSuchInstance(pe.getVersion());
						}
						SnmpGauge var5 = new SnmpGauge(((Long )toReturn).longValue());
			...
			varb.setObjectID(AgentUtil.getScalarSnmpOID(brunomateusOidRep, columnID));
		}
	}
}
